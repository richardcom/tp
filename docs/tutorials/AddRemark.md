...
This command allows users of the Addressbook application to add optional remarks to people in their intellibrary and edit it if required. The command should have the following format:
...
/**
 * Changes the remark of an existing book in the intellibrary.
 */
...
## Modify `Book` to support a `Remark` field

Since `BookCard` displays data from a `Book`, we need to update `Book` to get our `Remark` displayed!

### Modify `Book`

We change the constructor of `Book` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Book`

Unfortunately, a change to `Book` will cause other commands to break, you will have to modify these commands to use the updated `Book`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Book` class to find these commands.
...
:exclamation: You must delete Addressbook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause Addressbook to default to an empty intellibrary!
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Book`. Since all fields in a `Book` are immutable, we create a new instance of a `Book` with the values that we want and
save it with `Model#setBook()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Book: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Book: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
